...
/*
 * Modified to interface with python and to return RMSD value.
 *
*/
...
double wrmsd_kabsch(unsigned int size, float (*Xarray)[3], double (*Yarray)[3]) //, double *rmsd)
{
	...
	/* allocate GSL objects for Kabsch routine */
	gsl_matrix *X = gsl_matrix_alloc(size, 3); /* fragment to be moved */
	gsl_matrix *Y = gsl_matrix_alloc(size, 3); /* fragment to move to */
	...
	double xm_z = 0.;
	double rmsd = 0.;
	double varsum = 0.; /* sum of coordinate variances */

	/* cast numpy coordinate arrays into GSL matrix */
	/* both numpy and C use c contiguous arrays */
	for (i = 0; i < size; ++ i) {
		/* X */
		gsl_matrix_set(X,i,0, Xarray[i][0]);
		gsl_matrix_set(X,i,1, Xarray[i][1]);
		gsl_matrix_set(X,i,2, Xarray[i][2]);

		/* Y */
		gsl_matrix_set(Y,i,0, Yarray[i][0]);
		gsl_matrix_set(Y,i,1, Yarray[i][1]);
		gsl_matrix_set(Y,i,2, Yarray[i][2]);
	}

	/* call Kabsch routine to superimpose fragments X and Y */
    kabsch(size, X, Y, U, t, &s);

	/* compute corrdinate variance over all points */
	/* Note: The original coordinate values in X and Y
		are modified after the Kabsch call, therefore
		we revert back to the original Xarray and Yarray values. */
	for (i = 0, varsum = 0.; i < size; ++ i) {
		/* move X */
		xm_x =  Xarray[i][0] * gsl_matrix_get(U,0,0) +
				Xarray[i][1] * gsl_matrix_get(U,0,1) +
				Xarray[i][2] * gsl_matrix_get(U,0,2) +
				gsl_vector_get(t,0);

		xm_y =	Xarray[i][0] * gsl_matrix_get(U,1,0) +
				Xarray[i][1] * gsl_matrix_get(U,1,1) +
				Xarray[i][2] * gsl_matrix_get(U,1,2) +
				gsl_vector_get(t,1);

		xm_z =	Xarray[i][0]* gsl_matrix_get(U,2,0) +
			    Xarray[i][1] * gsl_matrix_get(U,2,1) +
				Xarray[i][2]* gsl_matrix_get(U,2,2) +
				gsl_vector_get(t,2);

		/* calculate coordinate variance sum between moved X and Y */
		varsum +=  pow(xm_x - Yarray[i][0], 2)
				 + pow(xm_y - Yarray[i][1], 2)
		 		 + pow(xm_z - Yarray[i][2], 2);
	}
	...
	rmsd = sqrt(varsum / size);
	return rmsd;
}

/*____________________________________________________________________________*/
/* Vector cross product: gsl does not provide it */
